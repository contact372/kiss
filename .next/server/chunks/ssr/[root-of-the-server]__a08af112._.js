...
/**
 * @fileOverview An AI flow that transforms two uploaded images into a short video of the people kissing realistically using Pollo.ai's Kling model.
 *
...
const GenerateKissVideoInputSchema = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$genkit$2f$lib$2f$common$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].object({
    ...
    image2_data_uri: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$genkit$2f$lib$2f$common$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].string().describe("The second image as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'.")
});
...
// Flow using Pollo.ai's Kling model directly with two image inputs.
async function videoGenerationFlow(input) {
    console.log('[FLOW_LOG] Starting videoGenerationFlow.');
    const videoPromptText = `Two real people from the input images, side by side, facing each other, natural lighting, gentle camera micro-motion, realistic facial expressions, short kiss, tasteful, 5 seconds.`;
    ...
    // 1. Submit video generation task to Pollo.ai
    let taskId;
    try {
        const url = 'https://api.pollo.ai/v1/tasks';
        const headers = {
        ...
        const body = {
            ...
            model_params: {
                ...
                image_urls: [
                    input.image1DataUri,
                    input.image2_data_uri
                ],
                duration: 5,
                aspect_ratio: "16:9"
            }
        };
        console.log(`[DEBUG] Submitting task to Pollo.ai...`);
        console.log(`[DEBUG]   URL: POST ${url}`);
        console.log(`[DEBUG]   Headers: ${JSON.stringify({
            ...headers,
            Authorization: 'Bearer [REDACTED]'
        })}`);
        console.log(`[DEBUG]   Body: ${JSON.stringify(body).substring(0, 200)}...`);
        const createResponse = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$node$2d$fetch$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"])(url, {
            method: 'POST',
            headers: headers,
        ...
        console.log(`[FLOW_LOG] Task created successfully. Task ID: ${taskId}`);
    } catch (error) {
        ...
        return {
            error: `Pollo.ai task submission failed: ${message}`
        };
    }
    ...
    try {
        ...
        let pollCount = 0;
        const maxPolls = 30; // 30 polls * 5 seconds = 150 seconds timeout
        let videoUrl;
        while((status === 'processing' || status === 'waiting') && pollCount < maxPolls){
            pollCount++;
            await new Promise((resolve)=>setTimeout(resolve, 5000)); // Wait 5 seconds
            const statusUrl = `https://api.pollo.ai/v1/tasks/${taskId}`;
            console.log(`[DEBUG] Polling task status (Attempt ${pollCount})...`);
            console.log(`[DEBUG]   URL: GET ${statusUrl}`);
            const statusResponse = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$node$2d$fetch$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"])(statusUrl, {
                headers: {
                    'Authorization': `Bearer ${apiKey}`
                }
            });
            if (!statusResponse.ok) {
                console.error(`[FLOW_WARN] Polling failed with status: ${statusResponse.status}`);
                continue; // Continue polling on transient errors
            }
        ...
        // 3. Download the video and convert to data URI
        console.log(`[DEBUG] Downloading generated video from URL: ${videoUrl}`);
        const videoDownloadResponse = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$node$2d$fetch$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["default"])(videoUrl);
        ...
        return {
            error: `Pollo.ai video generation failed: ${message}`
        };
    }
}
;
...
async function createKissVideoAction(input) {
    ...
    try {
        ...
        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$ai$2f$flows$2f$generate$2d$kiss$2d$video$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["generateKissVideo"])({
            ...
            image2_data_uri: input.image2_data_uri
        });
        ...
        if (message.includes('Could not refresh access token')) {
            return {
                error: 'A server authentication error occurred. Please try again later.'
            };
        }
        return {
            error: message
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a08af112._.js.map