...
/**
 * @fileOverview An AI flow that generates a video from a single combined image using Google's Veo model.
 *
...
async function generateKissVideo(input) {
    console.log('[FLOW_LOG] Starting generateKissVideo flow with Veo.');
    try {
        const result = await videoGenerationFlow(input);
        console.log('[FLOW_LOG] Veo videoGenerationFlow completed.');
        return result;
    } catch (e) {
...
async function videoGenerationFlow(input) {
    console.log('[VEO_FLOW] Starting Veo video generation.');
    try {
        // Step 1: Submit video generation task to Veo
        console.log('[VEO_FLOW] Submitting video generation task to Google AI.');
        let { operation } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$ai$2f$genkit$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["ai"].generate({
            ...
            prompt: [
                {
                    text: 'Make the people in the image kiss. The video should be cinematic, high quality, 4k.'
                },
                {
                    media: {
                        url: input.image1DataUri
                    }
                }
            ],
            config: {
        ...
        if (!operation) {
            console.error('[VEO_ERROR] Expected the model to return an operation, but it did not.');
            throw new Error('Video generation did not start correctly.');
        }
        console.log(`[VEO_FLOW] Video generation task started. Operation ID: ${operation.name}`);
        // Step 2: Poll for the video result
        let pollCount = 0;
        const maxPolls = 60; // 60 polls * 5 seconds = 5 minutes timeout
        while(!operation.done && pollCount < maxPolls){
            pollCount++;
            console.log(`[VEO_FLOW] Polling operation status (Attempt ${pollCount}). Done: ${operation.done}`);
        ...
        if (!operation.done) {
            console.error('[VEO_ERROR] Video generation timed out.');
            throw new Error('Video generation timed out after several minutes.');
        }
        if (operation.error) {
            console.error(`[VEO_ERROR] Video generation failed with an error: ${operation.error.message}`, operation.error);
            throw new Error(`Video generation failed: ${operation.error.message}`);
        }
        console.log('[VEO_FLOW] Operation completed successfully.');
        const video = operation.output?.message?.content.find((p)=>!!p.media);
        if (!video || !video.media?.url) {
            console.error('[VEO_ERROR] Operation succeeded but no video media was found in the response.');
            throw new Error('Operation succeeded but did not return a video.');
        }
        // The URL returned is a temporary Google Cloud Storage URL. We need to convert it to a data URI.
        console.log('[VEO_FLOW] Video media found. Downloading from temporary URL...');
        const videoDataUri = await convertUrlToDataUri(video.media.url);
        console.log('[VEO_FLOW] Video downloaded and converted to data URI.');
        return {
            videoDataUri
        };
    } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error during Veo video generation.";
        console.error(`[VEO_FATAL] Veo video generation failed: ${message}`, error);
        return {
            error: `Veo video generation failed: ${message}`
        };
    }
}
async function convertUrlToDataUri(url) {
    const { GoogleAuth } = await __turbopack_context__.r("[project]/node_modules/google-auth-library/build/src/index.js [app-rsc] (ecmascript, async loader)")(__turbopack_context__.i);
    ...
    if (!response.ok) {
        throw new Error(`Failed to download video from GCS. Status: ${response.status}`);
    }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__939ebb64._.js.map